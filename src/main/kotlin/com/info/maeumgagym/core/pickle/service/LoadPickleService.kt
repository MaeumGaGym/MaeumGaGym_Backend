...
import com.info.maeumgagym.core.pickle.model.Pickle.Companion.toResponse
import com.info.maeumgagym.core.pickle.port.`in`.LoadPickleFromIdUseCase
import com.info.maeumgagym.core.pickle.port.`in`.LoadPickleFromPoseUseCase
import com.info.maeumgagym.core.pickle.port.`in`.LoadRecommendationPicklesUseCase
...
internal class LoadPickleService(
    ...
    override fun loadRecommendationPickles(index: Int): PickleListResponse {
        ...
        if (allPickles.isEmpty() || allPickles.size <= index * INDEX_SIZE) {
            throw BusinessLogicException(204, "There's No Pickle")
        }
    ...
    override fun loadPickleFromId(id: String): PickleResponse =
        ...
        (readPicklePort.readById(id) ?: throw BusinessLogicException.PICKLE_NOT_FOUND)
    ...
    override fun loadAllPagedFromPose(poseId: Long, idx: Int, size: Int): PickleListResponse {
        // id로 자세 불러오기
        val pose = readPosePort.readById(poseId) ?: throw BusinessLogicException.POSE_NOT_FOUND
    ...
    private fun getRandomPickles(pickles: List<com.info.maeumgagym.core.pickle.model.Pickle>): List<com.info.maeumgagym.core.pickle.model.Pickle> =
        // 만약 랜덤 피클을 추출할 베이스 리스트의 크기가 추출할 크기보다 작다면 -> 베이스 리스트 그대로 반환
        pickles.takeIf { pickles.toSet().size <= INDEX_SIZE }
        // 아니라면 빈 Set을 생성해서 (중복 제거)
            ?: mutableSetOf<com.info.maeumgagym.core.pickle.model.Pickle>().apply {
